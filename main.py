...
class SelectStepDialog(ctk.CTkToplevel): 
    def __init__(self, parent, title: str, prompt: str, choices_dict: Dict[str, str]):
        super().__init__(parent)
        self.title(title)
    ...
    def _on_ok(self):
        ...
        self.result_id = self.choices_dict.get(selected_display_name)
        self.destroy()
...
class TestiOhjelmaApp:
    ...
    def _select_test_step_for_config(self, test_type_to_filter: str, window_title: str) -> Optional[str]:
        ...
        choices={f"{i+1}. {s['name']} (ID: ...{s['id'][-4:]})":s['id'] for i,s in enumerate(matching_steps)}
        return SelectStepDialog(self.root,window_title,f"Valitse muokattava '{AVAILABLE_TEST_TYPES.get(test_type_to_filter,test_type_to_filter)}'-vaihe:",choices).get_selected_id()
    ...
    def open_serial_config(self):
       selected_step_id = self._select_test_step_for_config('serial', "Sarjatestin Asetukset")
       if not selected_step_id: 
            return

       current_serial_settings = self.step_specific_settings.get(selected_step_id, self._get_default_settings_for_step_type('serial'))
       config_window = SerialConfigWindow(self.root, current_serial_settings)
       updated_settings = config_window.get_settings()
       if updated_settings is not None:
           self.step_specific_settings[selected_step_id] = updated_settings
           step_name = next((s['name'] for s in self.test_order if s['id'] == selected_step_id), "Sarjatesti")
           print(f"'{step_name}' asetukset päivitetty.")

    def open_daq_config(self):
        if not NIDAQMX_AVAILABLE: return
        selected_step_id = self._select_test_step_for_config('daq', "DAQ Testin Asetukset")
        if not selected_step_id:
            return
            
        current_daq_settings = self.step_specific_settings.get(selected_step_id, self._get_default_daq_settings())
        config_window = DAQConfigWindow(self.root, current_daq_settings)
        updated_settings = config_window.get_settings()
        if updated_settings is not None:
            self.step_specific_settings[selected_step_id] = updated_settings
            step_name = next((s['name'] for s in self.test_order if s['id'] == selected_step_id), "DAQ-testi")
            print(f"'{step_name}' asetukset päivitetty.")

    def open_modbus_config(self):
        if not PYMODBUS_AVAILABLE: return
        selected_step_id = self._select_test_step_for_config('modbus', "Modbus Testin Asetukset")
        if not selected_step_id:
            return

        current_modbus_sequence = self.step_specific_settings.get(selected_step_id, self._get_default_modbus_sequence())
        config_window = ModbusConfigWindow(self.root, current_modbus_sequence)
        updated_sequence = config_window.get_sequence()
        if updated_sequence is not None:
            self.step_specific_settings[selected_step_id] = updated_sequence
            step_name = next((s['name'] for s in self.test_order if s['id'] == selected_step_id), "Modbus-testi")
            print(f"'{step_name}' asetukset päivitetty.")
